...
const AdminReviews = () => {
  ...
  }, [user, currentPage, selectedItem, filterRating]);
  ...
  const fetchAllReviews = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      let url = `http://localhost:5000/api/reviews/all?page=${currentPage}&limit=20`;
      
      if (selectedItem) {
        url = `http://localhost:5000/api/reviews/item/${selectedItem}?page=${currentPage}&limit=20`;
      }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {reviews.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="mt-2 text-gray-500">No reviews match the current filter criteria.</p>
          </div>
        ) : (
          <>
